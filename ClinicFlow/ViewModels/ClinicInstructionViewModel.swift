...
class ClinicInstructionViewModel: ObservableObject {
    @Published private(set) var steps: [ClinicInstructionStep] = []
    ...
    private func loadSteps() {
        steps = ClinicInstructionDataProvider.steps(for: instructionName, room: roomNumber)
        currentStepIndex = 0
    }

    var currentStep: ClinicInstructionStep? {
